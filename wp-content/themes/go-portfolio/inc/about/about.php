...
{
	$hide_date = get_option('go_portfolio_infohide1');
	if (!empty($hide_date)) {
...
function go_portfolio_new_optins_texts_init()
{
	if (isset($_GET['xbnotice']) && $_GET['xbnotice'] == 1) {
		update_option('go_portfolio_infohide1', current_time('mysql'));
	}
}
